...
<div class="mb-4">
    ...
    <label for="{{$id}}" class="text-wrap block w-72 cursor-pointer rounded text-sm font-bold text-gray-700">
        {{ $label }}
        <p class="text-wrap mb-2 w-72 text-xs font-normal">{{ $description }}</p>
        <img id="previewImage{{ $id }}" class="h-24 w-24 rounded-lg"
...
<script>
    document.getElementById("{{ $id }}").addEventListener('change', function() {
        ...
        reader.onload = function() {
            ...
            var output = document.getElementById('previewImage{{ $id }}').src = reader.result;
        };
